...
fprintf('Best parameter vector w:\n');
disp(bestW');

fprintf('LaTeX model form:\n');
fprintf('\\hat{y} = \\sum_{k=0}^{%d} w_k T_k(\\tilde{x})\n', bestDegree);
fprintf('\\tilde{x} = \\frac{2(x - x_{\\min})}{x_{\\max} - x_{\\min}} - 1\n');

fprintf('LaTeX parameter vector:\n');
fprintf('\\mathbf{w} = [');
for k = 1:numel(bestW)
    if k < numel(bestW)
        fprintf('w_{%d}=%.6f, ', k - 1, bestW(k));
    else
        fprintf('w_{%d}=%.6f', k - 1, bestW(k));
    end
end
