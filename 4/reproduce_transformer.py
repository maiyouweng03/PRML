"""Mini reproduction of "Attention Is All You Need".

The script implements a compact Transformer encoder-decoder from the paper and
trains it on a synthetic sequence transduction task:

    source tokens:  [a, b, c, d, ...]
    target tokens:  [..., d, c, b, a, EOS]

It keeps the standard residual + layer norm structure and records whether the
model can learn the reverse-copy transduction task.

Run:
    python reproduce_transformer.py --quick
    python reproduce_transformer.py --steps 400 --seq-len 5
"""
...
def plot_reproduction_history(history: list[dict[str, float]], out_dir: Path) -> Path:
    """Draw a simple training curve without relying on matplotlib."""
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "reproduction_training_curve.png"
    ...
    draw.text((margin_l, 24), "Transformer Reproduction on Reverse-Copy Task", fill=(20, 20, 20), font=title_font)
    ...
    series = [
        ("token accuracy", "token_accuracy", (35, 110, 180)),
        ("sequence accuracy", "sequence_accuracy", (45, 150, 80)),
    ]
    ...
    lx, ly = 640, 110
    draw.rectangle((lx - 15, ly - 12, lx + 315, ly + 70), fill="white", outline=(230, 230, 230))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("results_reproduction"))
...
def main() -> None:
    ...
    final, history = train_one("transformer", True, cfg)
    results["transformer"] = final

    write_results(args.out_dir, cfg, results, history)
    figure_path = plot_reproduction_history(history, args.out_dir)
    print("\nFinal results:")
